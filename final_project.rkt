...
(check-expect (add-food (make-game (make-snake 'up (list (make-posn 1 1))) empty empty 1)
                        (make-posn 1 1))
...
; game-score : game -> nat
; This function computes the player’s score, based on the snake’s length and
; the time (ticks) taken to reach that length.
(define (game-score g)
...
(check-expect (compare-coord (make-posn 1 1) (list (make-posn 1 2) (make-posn 1 1)))
              true)
...
; returns true if the game is over. The game ends when the snake runs into
; itself, a wall, or an obstacle (purple dot).
(define (game-over? g)
  (cond
...
(check-expect (game-over? (make-game (make-snake empty (list (make-posn 2 2))) empty
                                     (list (make-posn 1 1)) 1))
              false)
(check-expect (game-over? (make-game (make-snake empty (list (make-posn 1 1))) empty
                                     (list (make-posn 1 1)) 1))
              true)

(check-expect (game-over? (make-game (make-snake empty (list (make-posn 20 20)))
                                     empty empty 1)) false)
(check-expect (game-over? (make-game (make-snake empty (list (make-posn 0 0)))
                                     empty empty 1)) true)
...
; This function moves the game forward one step. One step increments the game’s tick
; component and moves the snake, possibly causing it to eat and grow
